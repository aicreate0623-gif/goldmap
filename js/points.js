...
// ── ★評価ヘルパー ────────────────────────────
let _curStars = 0; // 編集中の一時評価値

function setStarRating(v){
  _curStars = (_curStars === v) ? 0 : v; // 同じ★を再タップで解除
...
function starsToHtml(v){
  ...
  const empty  = '☆'.repeat(3 - v);
  return `<span style="color:var(--gold-lt)">${filled}</span><span style="color:rgba(200,170,80,0.3)">${empty}</span>`;
}
