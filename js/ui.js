...
async function commShowGoldPrice(){
  ...
  try{
    ...
    localStorage.setItem(SK_GOLD_UI, JSON.stringify(cacheData));
    _renderGoldDisplay(cacheData);
  } catch(e){
    showToast('相場の取得に失敗しました。通信を確認してください。', 3000);
    console.warn('[GOLD]', e);
  }
...
function buildGeoLegend(){
  ...
  const tbody = document.getElementById('geo-legend-body');
  if(!tbody) return;
  if(tbody.children.length > 0) return; // 二重実行防止
  ...
  AGES.forEach(([label, code, period])=>{
    ...
    tdAge.style.cssText = 'padding:6px 5px;border:1px solid rgba(255,255,255,0.1);color:var(--txt);font-size:10px;font-weight:700;white-space:nowrap;vertical-align:middle;background:rgba(0,0,0,0.25);';
    ...
    cols.forEach((color, i)=>{
      ...
      td.style.cssText = 'padding:0;border:1px solid rgba(255,255,255,0.1);text-align:center;vertical-align:middle;min-width:54px;';
      if(color){
        const isGold = flags[i];
        td.innerHTML = `<div title="${ROCK_DESC[i]}" style="background:${color};height:34px;width:100%;display:flex;align-items:center;justify-content:center;font-size:${isGold?'13px':'9px'};color:rgba(0,0,0,0.65);font-weight:700;position:relative;cursor:default;">${isGold?'★':''}</div>`;
      } else {
  ...
  const extraItems = [
    {color:'#e85858', label:'活火山・火山岩'},
    {color:'#c8d8e8', label:'人工改変地・市街地'},
    {color:'#e0e8f0', label:'水域・海底'},
  ];
  const tdOtherHead = document.createElement('td');
  tdOtherHead.style.cssText = 'padding:5px;border:1px solid rgba(255,255,255,0.1);color:var(--txt-sub);font-size:9px;background:rgba(0,0,0,0.25);vertical-align:middle;';
  ...
  tdOtherVal.innerHTML = extraItems.map(({color,label})=>
    ...
    `<span style="display:inline-block;width:16px;height:16px;background:${color};border-radius:2px;border:1px solid rgba(0,0,0,0.2);flex-shrink:0;"></span>${label}</span>`
  ).join('');
