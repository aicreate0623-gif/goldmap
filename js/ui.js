...
function _dldGoStep(step){
  if(step===0){
    ...
    if(hint) hint.textContent = '指でドラッグすると矩形が描かれます。選択後に確定ボタンを押してください';
  }
  if(step===1 && _dldStep===2){
    // STEP2→STEP1へ戻る：範囲リセット
    _clearDrawPreview();
    _dldBounds = null;
    const ok   = document.getElementById('dld-draw-ok');
    const hint = document.getElementById('dld-draw-hint');
  ...
  if(step===2 && _dldStep===3){
    // STEP3→STEP2へ戻る：DL中は無効
    const backBtn = document.getElementById('dld-s3-back');
...
// ── STEP1に戻る（STEP2の戻るボタン用） ─────────────────
function _dldBack(){ _dldGoStep(1); }
...
//  ダイアログ
// ═══════════════════════════════════════════
const DLGS=['dlg-edit','dlg-savecf','dlg-detail','dlg-del','dlg-imp2','dlg-alr','dlg-contrib-on','dlg-contrib-off','dlg-premium-gate','dlg-gps-lost','dlg-cl-edit','dlg-cl-delete'];
