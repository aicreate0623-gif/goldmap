...
const MINES=[
  ...
  {lat:43.050,lng:141.983,name:'夕張川（夕張）',note:'北海道夕張市。北海道砂金産地リストに記載。河川蛇行部に砂金が集積しやすい地形が発達しており、採取報告も多数確認されている。信頼度：高。出典：北海道砂金採りスポット一覧（得北）'},
  ...
  // ── 追加 2025: 東北（宮城・岩手・秋田）──
  {lat:38.544,lng:141.130,name:'黄金山神社周辺沢（涌谷）',note:'宮城県涌谷町。奈良時代の産金記録が日本書紀に残る歴史的産金地。箟岳山麓一帯の江合川支流域。信頼度：高'},
  {lat:38.997,lng:141.108,name:'衣川合流域（北上川）',note:'岩手県奥州市衣川。奥州藤原氏の黄金文化を支えた砂金供給地として知られる。北上川本流との合流部付近。信頼度：高'},
  {lat:39.400,lng:140.400,name:'雄物川中流域（秋田）',note:'秋田県中央部。江戸期の採取記録が残るほか、現代の採取報告も複数あり。信頼度：中'},

  // ── 追加 2025: 北海道（石狩・天塩）──
  {lat:43.700,lng:142.900,name:'石狩川上流支流（上川）',note:'北海道上川地方。明治期のゴールドラッシュの記録と現代の採取報告が重なる産金地。信頼度：高'},
  {lat:44.900,lng:142.200,name:'天塩川中流（天塩）',note:'北海道天塩地方。道北の主要河川で砂金採取の報告あり。信頼度：中'},

  // ── 追加 2025: 新潟（佐渡）──
  {lat:38.050,lng:138.300,name:'西三川流域（佐渡）',note:'新潟県佐渡市。日本最大級の砂金山として知られる西三川砂金山の流域。千年を超える採掘の歴史を持つ。信頼度：高'},

  // ── 追加 2025: 関東・中部 ──
  {lat:35.500,lng:138.400,name:'富士川上流（山梨）',note:'山梨県。甲斐金山群に由来する砂金が富士川上流に流下。鉱山由来の産地として記録される。信頼度：中'},
  {lat:35.800,lng:139.000,name:'多摩川上流（奥多摩方面）',note:'東京都・山梨県境付近。現代の採取報告が複数あり。上流鉱山由来と見られる。信頼度：低'},
  {lat:34.900,lng:138.800,name:'狩野川支流（伊豆）',note:'静岡県伊豆地方。伊豆の金山鉱脈に由来する鉱脈系産地。信頼度：中'},

  // ── 追加 2025: 九州 ──
  {lat:31.900,lng:130.400,name:'川内川中流（薩摩川内）',note:'鹿児島県薩摩川内市。国内最大の金鉱山・菱刈鉱山の影響圏内にある河川。下流域への砂金流下が期待される。信頼度：中'},
...
function initGsjLayer(){
  // 起動時: インポートデータがあれば件数表示
  if(!db){ updMineCacheInfo(`状態: 組み込み${GSJ_MINE_DATA.length}件（取得ボタンで表示）`); return; }
  dbGetMine('gsj_imported').then(imported=>{
    if(imported && imported.data && imported.data.length > 0){
      updMineCacheInfo(`状態: 組み込み${GSJ_MINE_DATA.length}件 + インポート${imported.data.length}件（取得ボタンで表示）`);
      document.getElementById('btn-mine-load').textContent='📥 再取得';
    } else {
      updMineCacheInfo(`状態: 組み込み${GSJ_MINE_DATA.length}件（取得ボタンで表示）`);
    }
  }).catch(()=>{ updMineCacheInfo(`状態: 組み込み${GSJ_MINE_DATA.length}件`); });
}

// ── 取得ボタン: バッチ処理でプログレス表示しながらレイヤー構築

async function importMineGeoJSON(evt){
  ...
  try{
    ...
    json.features.forEach(f=>{
      ...
      const name=p.mine_name||p.name||p.mineName||'不明';
      const mat=p.material||p.mat||'Au_Ag';
      const cat=p.mine_type||p.cat||'metal';
      const trace=!!(p.trace||false);
      const mapsheet=p.mapsheet||p.map_name||'インポート';
      const legend=p.legend||p.material_type||'';
      extra.push({lat,lng,name,cat,mat,trace,mapsheet,legend});
    });
    if(!extra.length) throw new Error('ポイントデータが見つかりません');
    // IndexedDBに保存（既存インポートに追記）
    if(db){
      try{
        const existing = await dbGetMine('gsj_imported');
        const prev = (existing && existing.data) ? existing.data : [];
        const merged = prev.concat(extra);
        const now = new Date().toLocaleString('ja-JP',{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'});
        await dbPutMine('gsj_imported',{data:merged, count:merged.length, savedAt:now});
      }catch(e){}
    }
    await loadMineData();
    showAlert('完了',`${extra.length}件を追加しました`);
  }catch(e){
    showAlert('エラー','GeoJSONの読み込みに失敗: '+e.message);
  }
}

async function loadMineData(fromButton=false){
  const btn = document.getElementById('btn-mine-load');
  ...
  // 組み込みデータを常にベースとして使用
  // GeoJSONインポート分はIndexedDBに別途保存 → 合算して表示
  let data = [...GSJ_MINE_DATA];
  let importCount = 0;
  if(db){
    try{
      const imported = await dbGetMine('gsj_imported');
      if(imported && imported.data && imported.data.length > 0){
        importCount = imported.data.length;
        data = data.concat(imported.data);
      }
    }catch(e){}
  }
  ...
  btn.disabled = false;
  btn.textContent = '📥 再取得';
  const cnt = gsjLayer.getLayers().length;
  const importMsg = importCount > 0 ? ` + インポート${importCount}件` : '';
  updMineCacheInfo(`状態: 表示中 ${cnt}件（組み込み${GSJ_MINE_DATA.length}件${importMsg}）`);
  if(fromButton && curTab==='offline') switchTab('map');
}
...
function toggleGsjLayer(){
  ...
  if(gsjVisible){
    ...
    if(gsjLayer){
      ...
      // 未読込 → オフラインシートへ誘導
      updMineCacheInfo('状態: 未読込 ← オフラインタブの「取得」ボタンで読込');
    }
  } else {
...
async function clearMineCache(){
  if(!confirm('インポートしたGeoJSONデータを削除しますか？\n（組み込みデータは削除されません）')) return;
  if(db){ try{ await dbDelMine('gsj_imported'); }catch(e){} }
  // 組み込みデータのみで再描画
  await loadMineData();
  showAlert('完了','インポートデータを削除しました\n組み込み'+GSJ_MINE_DATA.length+'件で表示中');
}

function updMineCacheInfo(msg){
  const el=document.getElementById('mine-cache-info');
  if(el) el.innerHTML='<b>状態:</b> '+msg;
}
